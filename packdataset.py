...
def pack(dsdir, outdir, outname):
    ...
    for f in os.listdir(dsdir):
        val_recordings.append(('data', os.path.join(dsdir, f)))
        
    splits['val']=val_recordings
    splits['train']=[]
    
    flist=[f for f in os.listdir(dsdir)]
    # Now we need to sort flist by value:
    flist=sorted(flist, key=sortKey)
    flist=[os.path.join(dsdir, f) for f in flist]
    
    vals=np.zeros((len(flist),)+desired_im_sz+(3,),np.uint8)
    srclist=["s" for f in flist]
    for i, f in enumerate(flist):
        im=imread(f)
        vals[i]=process_im(im, desired_im_sz)
        
    hkl.dump(vals, os.path.join(outdir, outname+".hkl"))
    hkl.dump(srclist, os.path.join(outdir, "sources_"+outname+".hkl"))  


if __name__ == '__main__':
    if len(sys.argv)!=4:
        print("Usage: packdataset.py datsetdir outpath outname")
    else:
        dsdir=sys.argv[1]
        outpath=sys.argv[2]
        outname=sys.argv[3]
        if not(os.path.exists(dsdir)):
            print("Error: dataset directory doesn't exist!")
            sys.exit(1)
        
        # For now, all data will be in the testing split:
        pack(dsdir, outpath, outname)
