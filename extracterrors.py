...
# Now that we're here, let's set some filenames:
nt=1700 # nt represents the total number of images to compute.
batch_size=100 # This is the number of elements in a prediction run
n_plot=100 # This value only seems to change the number of images or perhaps samples plotted in the old code path.
if __name__=="__main__":
    print("Setting up...")
    if len(sys.argv)!=4:
        print("Usage: extracterrors.py inname outname layerdesignation")
        sys.exit(1)
    ...
    input_shape[0]=nt # The nt parameter appears to be... uhhh... the number of elements in... something.
    inputs=Input(shape=tuple(input_shape))
    predictions=test_prednet(inputs)
    test_model=Model(inputs=inputs, outputs=predictions)
    test_generator=SequenceGenerator(testfile, srcfile, nt, sequence_start_mode="unique", data_format=data_format)
    ...
    hickle.dump(predictions, outname)
